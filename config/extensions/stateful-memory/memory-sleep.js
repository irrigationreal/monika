...
function buildWakeTask({ recencyIndexFile, observationsFile, wakeFile }) {
  return [
    ...
    "This is the entity context file — a compact summary of recent observations and",
    "tracked entities from the memstore observation store. It shows recent observations",
    "(the last 15) with truncated bodies, plus a list of other known entities.",
    "",
    "Step 3: Use the recall tool to fill in recent activity.",
    "Based on what you see in the entity context (active projects, recent observations),",
    ...
    "timestamps to orient around. The recall tool searches both session transcripts",
    "and entity observations — use it to get the full detail behind the truncated",
...
function buildFactsTask({ observationsFile, factsFile }) {
  return [
    ...
    "",
    "Step 2: Read the entity context for current state.",
    `Read the file at: ${observationsFile}`,
    "This contains recent entity observations and a list of all tracked entities,",
    "rendered from the memstore observation store. Use the recall tool to get full",
    "detail on any entity or topic that seems relevant to FACTS.md curation.",
    "",
    ...
    "",
    "That dynamic state is now covered by entity observations (searchable via recall)",
    "and WAKE.md (temporal orientation). FACTS.md is the stable foundation beneath both.",
    "",
    "Keep it tight. If something is already well-represented in entity observations,",
