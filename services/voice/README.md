...
The BFF exposes only the lab's login/session, Realtime connect/disconnect and
...
Audio then flows directly between WebRTC peers.
...
`canonical: false`. The browser records captions, status, interruption, usage,
and bounded errors. Users can export or delete each record.
