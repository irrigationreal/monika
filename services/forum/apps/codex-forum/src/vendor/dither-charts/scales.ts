import { scaleBand, scaleLinear, scalePoint } from 'd3-scale';
import { stack as d3Stack, stackOffsetExpand } from 'd3-shape';

export type StackType = 'default' | 'stacked' | 'percent';
export type ChartRow = Record<string, unknown>;

const finiteNumber = (value: unknown) => (typeof value === 'number' && Number.isFinite(value) ? value : 0);

/** Adapted from Dither UI's shared cartesian scale helpers. */
export function computeBands(
  data: ChartRow[],
  keys: string[],
  stackType: StackType
): { bands: Record<string, [number, number][]>; max: number } {
  if (stackType === 'default') {
    const bands: Record<string, [number, number][]> = {};
    let max = 0;
    for (const key of keys) {
      bands[key] = data.map((row) => {
        const value = finiteNumber(row[key]);
        max = Math.max(max, value);
        return [0, value];
      });
    }
    return { bands, max: max || 1 };
  }

  const series = d3Stack<ChartRow>()
    .keys(keys)
    .value((row, key) => finiteNumber(row[key]))
    .offset(stackType === 'percent' ? stackOffsetExpand : (undefined as never))(data);

  const bands: Record<string, [number, number][]> = {};
  let max = 0;
  series.forEach((layer) => {
    bands[layer.key] = layer.map((point) => {
      max = Math.max(max, point[1]);
      return [point[0], point[1]];
    });
  });
  return { bands, max: max || 1 };
}

export function buildXScale(length: number, plotWidth: number) {
  return scalePoint<number>()
    .domain(Array.from({ length }, (_, index) => index))
    .range([0, plotWidth]);
}

export function buildBandScale(length: number, plotWidth: number, gap = 0.28, edge = 0.18) {
  return scaleBand<number>()
    .domain(Array.from({ length }, (_, index) => index))
    .range([0, plotWidth])
    .paddingInner(Math.max(0, Math.min(0.9, gap)))
    .paddingOuter(Math.max(0, Math.min(2, edge)));
}

export function buildYScale(max: number, plotHeight: number) {
  return scaleLinear().domain([0, max]).nice().range([plotHeight, 0]);
}
