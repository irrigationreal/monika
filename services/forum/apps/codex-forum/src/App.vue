...
const breadcrumbs = computed((): BreadcrumbItem[] => {
  const crumbs: BreadcrumbItem[] = [
    { label: 'Forum Home', to: { name: 'forum.home' } }
  ];

  const isForumRoute = route.name === 'forum.view' || route.name === 'forum.newthread';
  const isTopicRoute =
    route.name === 'topic.view' || route.name === 'topic.state' || route.name === 'session.inspect';
  const forumId = isForumRoute
    ? ((route.params['forumId'] as string | undefined) ?? null)
    : (isTopicRoute ? state.selectedTopic.value?.forumId ?? null : null);
    ...
    <div class="vb-shell">
      <header class="vb-header">
        ...
        <div class="vb-welcome">
          <div class="vb-welcome-summary">
            <span class="vb-welcome-main">Welcome, <strong>{{ displayName }}</strong>.</span>
          ...
          <div class="vb-welcome-links">
            ...
            <router-link :to="{ name: 'chat.home' }">Chat</router-link>
            <router-link :to="{ name: 'developer.portal' }">Developers</router-link>
      ...
      <nav class="vb-nav">
        ...
        <div class="vb-nav-items" :class="{ 'vb-nav-open': mobileMenuOpen }">
          <router-link class="vb-nav-item" :to="{ name: 'forum.home' }" @click="closeMobileMenu">Forum Home</router-link>
          <router-link v-if="state.isLoggedIn.value" class="vb-nav-item" :to="{ name: 'user.profile' }" @click="closeMobileMenu">Profile</router-link>
          <router-link v-if="state.isLoggedIn.value" class="vb-nav-item" :to="{ name: 'user.files' }" @click="closeMobileMenu">Files</router-link>
          <router-link class="vb-nav-item" :to="{ name: 'chat.home' }" @click="closeMobileMenu">Chat</router-link>
