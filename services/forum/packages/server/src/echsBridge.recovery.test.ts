...
describe('passive ECHS startup reconciliation', () => {
  ...
  it('retries cancellation once with the same durable operation identity inside the bounded deadline', async () => {
    ...
    const request = vi.spyOn(client as any, 'request')
      ...
      .mockResolvedValueOnce({ ok: true, operation_id: 'op-1', generation: 4, state: 'stopped', targets: 0, unresolved_count: 0, effects_unknown_count: 0, error_count: 0, message: 'stopped' });
    await expect(client.cancelPiSession('parent', { operationId: 'op-1', generation: 4 })).resolves.toMatchObject({ state: 'stopped' });
    expect(request).toHaveBeenCalledTimes(2);
    expect(request.mock.calls[0]).toEqual(request.mock.calls[1]);
    expect(request.mock.calls[0]?.[1]).toMatchObject({ timeoutMs: 10_000, body: { operation_id: 'op-1', generation: 4 } });
  });
  ...
  it('reconciles unresolved canonical cancellation without requiring a loaded conversation', async () => {
    ...
    store.listPiSessionLinksWithUnresolvedCancellation.mockReturnValue([{ pi_session_id: 'pi-parent', topic_id: 'topic-1' }]);
    vi.spyOn((bridge as any).client, 'reconcilePiSessionCancellation').mockResolvedValue({ ok: false, operation_id: 'op-1', generation: 4,
      state: 'stopping', targets: 1, unresolved_count: 1, effects_unknown_count: 0, error_count: 0, message: 'stopping' });
  ...
  it.each(['stopping', 'uncertain'] as const)('does not overwrite %s with thinking during active startup reconciliation', async (activity) => {
    const { bridge, store } = bridgeFixture({ conversation_id: 'conversation-1', active_thread_id: 'conversation-1', activity: 'active' });
  ...
  it('interrupt fences durable work and clears a queued local turn even without a loaded thread', async () => {
    const store = {
      ...
      getPiSessionLinkByTopic: vi.fn(() => null), getRobotState: vi.fn(() => null),
      advanceTopicDispatchGeneration: vi.fn(() => ({ generation: 3, cancelled: 1 })),
      getTopicDispatchGeneration: vi.fn(() => 3), isTopicDispatchGenerationCurrent: vi.fn(() => true), setRobotActivity: vi.fn(),
    };
    const bridge = new EchsBridge(store as any, { emit: vi.fn(), subscribe: vi.fn() } as any, {
      model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    });
  ...
  it('publishes generation fence before awaiting agentd interrupt and sends that generation', async () => {
    ...
    const store = {
      ...
      getPiSessionLinkByTopic: vi.fn(() => null), getRobotState: vi.fn(() => ({ activity: 'thinking' })),
      advanceTopicDispatchGeneration: vi.fn(() => { order.push('fence'); return { generation: 8, cancelled: 1 }; }),
      getTopicDispatchGeneration: vi.fn(() => 8), isTopicDispatchGenerationCurrent: vi.fn(() => true), setRobotActivity: vi.fn(),
    };
    const bridge = new EchsBridge(store as any, { emit: vi.fn(), subscribe: vi.fn() } as any, {
      model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    });
    ...
    vi.spyOn((bridge as any).client, 'interruptConversation').mockImplementation(async (_id: string, generation: number, operationId: string) => {
      ...
      return { ok: true, operation_id: operationId, generation, state: 'stopped', targets: 0, unresolved_count: 0, effects_unknown_count: 0, error_count: 0, message: 'stopped' };
    });
  ...
  it('emits exactly one interrupted reset before unloaded stop can publish idle', async () => {
    ...
    const store = {
      getSessionByTopic: vi.fn(() => ({ agent_thread_id: null })),
      getPiSessionLinkByTopic: vi.fn(() => ({ pi_session_id: 'pi-parent' })), getRobotState: vi.fn(() => ({ activity: 'thinking' })),
      advanceTopicDispatchGeneration: vi.fn(() => ({ generation: 9, cancelled: 0 })),
      getTopicDispatchGeneration: vi.fn(() => 9), isTopicDispatchGenerationCurrent: vi.fn(() => true),
      setRobotActivity: vi.fn((_topicId: string, activity: string) => { if (activity === 'stopped') order.push('stopped'); }),
    };
    const bus = { emit: vi.fn((_topicId: string, event: { type: string }) => { if (event.type === 'assistant_reset') order.push('reset'); }), subscribe: vi.fn() };
    const bridge = new EchsBridge(store as any, bus as any, { model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' } });
    vi.spyOn(bridge as any, 'emitState').mockImplementation(() => {});
    vi.spyOn((bridge as any).client, 'cancelPiSession').mockResolvedValue({ ok: true, operation_id: 'op', generation: 9,
      state: 'stopped', targets: 0, unresolved_count: 0, effects_unknown_count: 0, error_count: 0, message: 'stopped' });
  ...
  it('stops by canonical Pi session when the parent conversation is unloaded', async () => {
    const store = {
      ...
      getPiSessionLinkByTopic: vi.fn(() => ({ pi_session_id: 'pi-parent' })), getRobotState: vi.fn(() => ({ activity: 'thinking' })),
      advanceTopicDispatchGeneration: vi.fn(() => ({ generation: 9, cancelled: 0 })),
      getTopicDispatchGeneration: vi.fn(() => 9), isTopicDispatchGenerationCurrent: vi.fn(() => true), setRobotActivity: vi.fn(),
    };
    const bridge = new EchsBridge(store as any, { emit: vi.fn(), subscribe: vi.fn() } as any, {
      model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    });
    vi.spyOn(bridge as any, 'emitState').mockImplementation(() => {});
    const cancel = vi.spyOn((bridge as any).client, 'cancelPiSession').mockResolvedValue({ ok: true, operation_id: 'op', generation: 9,
      state: 'stopped', targets: 2, unresolved_count: 0, effects_unknown_count: 0, error_count: 0, message: 'stopped' });
  ...
  it.each(['stopping', 'uncertain'] as const)('blocks queue and steer service paths while activity is %s', async (activity) => {
    ...
    expect(enqueue).not.toHaveBeenCalled(); expect(store.ensureSession).not.toHaveBeenCalled();
  });
  ...
  it('keeps the forum fence visibly uncertain after an agentd deadline', async () => {
    const store = {
      ...
      getTopicDispatchGeneration: vi.fn(() => 10), isTopicDispatchGenerationCurrent: vi.fn(() => true), setRobotActivity: vi.fn(),
    };
    const bridge = new EchsBridge(store as any, { emit: vi.fn(), subscribe: vi.fn() } as any, {
      model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    });
    vi.spyOn(bridge as any, 'emitState').mockImplementation(() => {});
    vi.spyOn((bridge as any).client, 'cancelPiSession').mockRejectedValue(new DOMException('timed out', 'TimeoutError'));
    await expect(bridge.interruptTopic('topic-1')).resolves.toMatchObject({ ok: false, state: 'uncertain', generation: 10 });
  ...
  it('ignores out-of-order turn_interrupted SSE and preserves current typed stopped state', () => {
    ...
    const store = {
      ...
      upsertRobotState: vi.fn((input: { activity: string }) => { activity = input.activity; }),
    };
    const bus = { emit: vi.fn(), subscribe: vi.fn() };
    const bridge = new EchsBridge(store as any, bus as any, {
      model: 'model', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    ...
    (bridge as any).handleEvent('conversation-1', {
      event: 'turn_interrupted', data: { generation: 1, operation_id: 'older', cancellation: { state: 'stopped' } },
    });
    ...
    (bridge as any).handleEvent('conversation-1', {
      event: 'turn_interrupted', data: { generation, operation_id: 'current', cancellation: { state: 'stopped' } },
    });
