...
describe('Forum routes silent posts', () => {
  ...
  it('rolls back an acquired-fence reply before any post or dispatch becomes visible', async () => {
    const { app } = await buildApp();
    const forum = store.createForum('Forum', null, null, null, null, 'active', 'public');
    const author = store.createIdentityWithPassword('Author', 'human', 'pw-hash', 'author');
    const token = 'author-token';
    store.createAuthSession(token, author.id);
    const { topic } = store.createTopic({ forumId: forum.id, title: 'Topic', body: 'opening', authorId: author.id });
    const before = store.countPostsByTopic(topic.id);
    const fenced = Object.assign(new Error('Robot work admission is temporarily closed for deployment'), {
      statusCode: 503,
      retryAfter: 1,
    });
    ...
    const response = await app.inject({
      ...
      url: `/topics/${topic.id}/posts`,
      headers: { authorization: `Bearer ${token}` },
      payload: { body: 'must retry' },
    });

    expect(response.statusCode).toBe(503);
    expect(store.countPostsByTopic(topic.id)).toBe(before);
    expect(countDispatches()).toBe(0);
  });

  it('keeps an explicit-dispatch post silent when durable dispatch creation fails', async () => {
    const { app } = await buildApp();
    const forum = store.createForum('Forum', null, null, null, null, 'active', 'public');
    const author = store.createIdentityWithPassword('Author', 'human', 'pw-hash', 'author');
    const token = 'author-token';
    ...
    const opening = store.listPosts(topic.id)[0]!;
    const createDispatch = vi.spyOn(store, 'createPostDispatch').mockImplementation(() => {
      throw new Error('injected dispatch failure');
    });

    const response = await app.inject({
      method: 'POST',
      url: `/posts/${opening.id}/dispatch`,
      headers: { authorization: `Bearer ${token}` },
      payload: {},
    });

    expect(response.statusCode).toBe(500);
    expect(store.getPost(opening.id)?.silent).toBe(1);
    expect(countDispatches()).toBe(0);
  ...
  it('blocks handoff publication and launch while deployment admission is acquired', async () => {
    const { app, codex } = await buildApp();
    const forum = store.createForum('Forum', null, null, null, null, 'active', 'public');
    const author = store.createIdentityWithPassword('Author', 'human', 'pw-hash', 'author');
    const token = 'author-token';
    ...
    await coordinator.acquire({ operationId: 'handoff-fence', waitTimeoutMs: 100, leaseMs: 60_000 });
    ...
    expect(response.statusCode).toBe(503);
    expect((db.prepare('select count(*) as count from topics').get() as { count: number }).count).toBe(topicCount);
    expect(codex.createLinkedHandoffConversation).not.toHaveBeenCalled();
    expect(codex.sendUserMessage).not.toHaveBeenCalled();
    coordinator.close();
  });

  it('keeps deployment acquisition blocked during delayed linked-handoff creation', async () => {
    let resolveLinked!: () => void;
    const linkedStarted = Promise.withResolvers<void>();
    const { app } = await buildApp({
      createLinkedHandoffConversation: vi.fn(async () => {
        const release = store.beginRobotWork();
        linkedStarted.resolve();
        try {
          await new Promise<void>((resolve) => (resolveLinked = resolve));
        } finally {
          release();
        }
      }),
    ...
    const sourceSession = store.ensureSession({ topicId: sourceTopic.id });
    store.upsertPiSessionLink({
      piSessionId: 'source-session-delayed',
      piSessionPath: '/tmp/source-session-delayed.jsonl',
      topicId: sourceTopic.id,
      sessionId: sourceSession.id,
      kind: 'normal',
    });
    ...
    const handoff = app.inject({
      method: 'POST',
      url: `/topics/${sourceTopic.id}/handoff`,
      headers: { authorization: `Bearer ${token}` },
      payload: { title: 'Delayed handoff', draft: 'Continue safely.' },
    });
    await linkedStarted.promise;
    const admission = await coordinator.acquire({
      operationId: 'deploy-during-handoff',
      waitTimeoutMs: 100,
      leaseMs: 60_000,
    });
    ...
    resolveLinked();
    expect((await handoff).statusCode).toBe(200);
    coordinator.close();
  });
