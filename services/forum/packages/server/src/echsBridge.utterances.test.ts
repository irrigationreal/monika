...
describe('ECHS canonical utterance projection', () => {
  ...
  it('preserves grouped contributor provenance through the queued turn sent to agentd', async () => {
    ...
    const { topic, post: first } = store.createTopic({
      forumId: forum.id, title: 'Topic', body: 'First', authorId: human.id,
    });
    ...
    store.upsertPiSessionLink({
      piSessionId: 'pi-parent', piSessionPath: '/tmp/parent.jsonl', topicId: topic.id, sessionId: session.id,
    });
    const origin = {
    ...
    const bridge = new EchsBridge(store, { emit: vi.fn(), subscribe: vi.fn() } as any, {
      model: 'm', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
    });
    ...
    const enqueue = vi.spyOn((bridge as any).client, 'enqueueConversationMessage').mockResolvedValue({
      messageId: 'dispatch', threadId: 'thread', deduplicated: false,
    });

    await bridge.dispatchPostToAgent(topic.id, trigger.id, {
      dispatchId: 'dispatch', generation: 0, contributorPostIds: [first.id, trigger.id], origin,
    });

    expect(enqueue).toHaveBeenCalledWith('conversation', expect.any(String), expect.objectContaining({
      provenance: {
        origin: 'forum', topicId: topic.id, postId: trigger.id, version: 2,
        utteranceIds: [first.id, trigger.id], executionOrigins: [origin],
      },
    }));
  });
  ...
  it('cancels the delayed completion backfill when stopped before its timer fires', async () => {
    ...
    try {
      const forum = store.createForum('Forum');
      const human = store.createIdentity('Author', 'human');
      const { topic, post } = store.createTopic({ forumId: forum.id, title: 'Topic', body: 'Work', authorId: human.id });
      const session = store.ensureSession({ topicId: topic.id });
      store.upsertRobotState({
        topicId: topic.id, sessionId: session.id, activity: 'thinking', model: 'm', reasoningEffort: null, currentPlanId: null,
      });
      const bridge = new EchsBridge(store, { emit: vi.fn(), subscribe: vi.fn() } as any, {
        model: 'm', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' },
      });
      ...
      (bridge as any).threadMap.set('conversation', {
        topicId: topic.id, sessionId: session.id, activeThreadId: 'thread', lastUserPostId: post.id,
        turnParentPostId: post.id, planId: null, reasoningSummary: '', reasoningBackfillAttempted: false,
        reasoningBackfillRetries: 0, model: 'm', reasoningEffort: null, currentTurnId: 'turn', turnStartedAt: Date.now(),
        lastUsage: null, totalInputTokens: 0, totalOutputTokens: 0, activeSubagents: new Map(),
        lastStreamEventAt: null, reasoningCheckpoints: [], assistantText: '', assistantCheckpoints: [],
      });
  ...
  it('serializes multiple canonical items before turn idle and freezes continuation per item', async () => {
    ...
    const { topic, post: origin } = store.createTopic({
      forumId: forum.id, title: 'Topic', body: 'Start work', authorId: human.id,
    });
    const session = store.ensureSession({ topicId: topic.id });
    store.upsertPiSessionLink({
      piSessionId: 'pi-parent', piSessionPath: '/tmp/parent.jsonl', topicId: topic.id, sessionId: session.id,
    ...
    const bridge = new EchsBridge(store, bus as any, { model: 'm', workDir: '/tmp', echs: { baseUrl: 'http://agentd.invalid' } });
    ...
    vi.spyOn(bridge as any, 'emitContext').mockResolvedValue(undefined);
    (bridge as any).threadMap.set('conversation', {
      topicId: topic.id, sessionId: session.id, activeThreadId: 'thread', lastUserPostId: origin.id,
      turnParentPostId: origin.id, planId: null, reasoningSummary: '', reasoningBackfillAttempted: false,
      reasoningBackfillRetries: 0, model: 'm', reasoningEffort: null, currentTurnId: 'turn', turnStartedAt: Date.now(),
      lastUsage: null, totalInputTokens: 0, totalOutputTokens: 0, activeSubagents: new Map(),
      lastStreamEventAt: null, reasoningCheckpoints: [], assistantText: '', assistantCheckpoints: [],
    });
    ...
    (bridge as any).handleEvent('conversation', {
      event: 'item_completed', data: {
        item: { id: 'pi-a', pi_message_id: 'pi-a', utterance_id: 'pi-a', type: 'message', role: 'assistant', content: [{ type: 'text', text: 'Ordinary answer' }] },
      },
    });
    (bridge as any).handleEvent('conversation', {
      event: 'item_completed', data: {
        item: {
          id: 'pi-b', pi_message_id: 'pi-b', utterance_id: 'pi-b', type: 'message', role: 'assistant',
          content: [{ type: 'text', text: 'Background answer' }], source_kind: 'subagent-completion',
          subagent_run_id: 'run-1', origin_post_id: origin.id, origin_topic_id: topic.id,
        },
      },
    });
    ...
    const pending = store.createPendingAttachment({
      topicId: topic.id, filename: 'canonical.txt', mimeType: 'text/plain', sizeBytes: attachmentBytes.length,
      storagePath: attachmentPath, sha256: createHash('sha256').update(attachmentBytes).digest('hex'),
    ...
    const legacyPending = store.createPendingAttachment({
      topicId: topic.id, filename: 'legacy.txt', mimeType: 'text/plain', sizeBytes: attachmentBytes.length,
      storagePath: legacyPath, sha256: createHash('sha256').update(attachmentBytes).digest('hex'),
      expiresAt: new Date(Date.now() + 60_000).toISOString(),
    });
    (bridge as any).handleEvent('conversation', {
      event: 'item_completed', data: {
        item: {
          id: 'pi-c', pi_message_id: 'pi-c', utterance_id: 'pi-c', type: 'message', role: 'assistant',
          content: [{ type: 'text', text: `Attachment answer\n[forum-attachment id="${legacyPending.id}"]` }],
          attachment_refs: [{
            refEntryId: 'canonical-custom-entry-id', pendingAttachmentId: pending.id,
            sha256: createHash('sha256').update(attachmentBytes).digest('hex'), sizeBytes: attachmentBytes.length,
          }],
        },
      },
    });
    ...
    const posts = db.prepare("select id, body, parent_post_id from posts where body in ('Ordinary answer', 'Background answer', 'Attachment answer') order by rowid").all() as Array<any>;
    ...
    expect(store.listAttachmentHandoffsForProjection(store.getAssistantProjection('pi-parent', 'pi-c')!.id)[0]?.ref_entry_id)
    ...
    expect(bus.emit).toHaveBeenCalledWith(topic.id, expect.objectContaining({
      type: 'assistant_message', data: expect.objectContaining({ piMessageId: 'pi-b', utteranceId: 'pi-b' }),
    }));
  });
  ...
  it('recovers every lost canonical sibling after one projected live and replays each identity exactly once', async () => {
    ...
    try {
      ...
      store.createIdentity('Monika', 'robot');
      const { topic, post: origin } = store.createTopic({
        forumId: forum.id, title: 'Topic', body: 'Background request', authorId: human.id,
      });
      const session = store.ensureSession({ topicId: topic.id });
      store.upsertPiSessionLink({
        piSessionId: 'pi-parent', piSessionPath: '/tmp/parent.jsonl', topicId: topic.id, sessionId: session.id,
      ...
      writeFileSync(attachmentPath, attachmentBytes);
      const pending = store.createPendingAttachment({
        topicId: topic.id, filename: 'recovered.txt', mimeType: 'text/plain', sizeBytes: attachmentBytes.length,
        storagePath: attachmentPath, sha256: createHash('sha256').update(attachmentBytes).digest('hex'),
        expiresAt: new Date(Date.now() + 60_000).toISOString(),
      });
      const executionOrigin = {
        utteranceId: 'external-utterance', originKind: 'external', channelKind: 'discord',
        topicId: topic.id, postId: origin.id, surfaceId: 'channel-1', externalEventId: 'event-1',
        scope: 'thread-1', scopeKind: 'thread',
      };
      const canonicalData = {
        piMessageId: ' pi-recovered ',
        item: {
          id: 'adapter-item-id', type: 'message', message_kind: 'assistant_outward', role: 'assistant',
          utterance_id: 'utterance-recovered', content: [{ type: 'text', text: 'Recovered background result' }],
          source_kind: 'subagent-completion', subagent_run_id: 'run-recovered',
          origin_post_id: origin.id, origin_topic_id: topic.id,
          execution_origins: [executionOrigin],
          attachment_refs: [{
            refEntryId: 'recovered-ref', pendingAttachmentId: pending.id, sha256: pending.sha256,
      ...
      const liveAData = {
        item: {
          id: 'pi-live-a', type: 'message', message_kind: 'assistant_outward', role: 'assistant',
          content: [{ type: 'text', text: 'Live A' }],
        },
      };
      const lostCData = {
        item: {
          id: 'pi-lost-c', type: 'message', message_kind: 'assistant_outward', role: 'assistant',
      ...
      const context = {
        topicId: topic.id, sessionId: session.id, activeThreadId: 'thread', lastUserPostId: origin.id,
        turnParentPostId: origin.id, planId: null, reasoningSummary: '', reasoningBackfillAttempted: false,
        reasoningBackfillRetries: 0, model: 'm', reasoningEffort: null, currentTurnId: 'turn', turnStartedAt,
        lastUsage: null, totalInputTokens: 0, totalOutputTokens: 0, activeSubagents: new Map(),
        lastStreamEventAt: null, reasoningCheckpoints: [], assistantText: '', assistantCheckpoints: [],
      };
      ...
      (bridge as any).handleEvent('conversation', {
        event: 'turn_completed', data: { pi_message_id: 'pi-lost-c' },
      });
      await Promise.resolve();
      await vi.advanceTimersByTimeAsync(1000);
      ...
      expect(projectedPost).toMatchObject({
        body: 'Recovered background result', parent_post_id: origin.id, follow_up: 1,
      });
      ...
      expect(JSON.parse(store.getPiMessageLink('pi-parent', 'pi-recovered')?.metadata_json ?? '{}')).toMatchObject({
        sourceKind: 'subagent-completion', runId: 'run-recovered', originPostId: origin.id, originTopicId: topic.id,
      });
      const orderedPosts = db.prepare(
      ...
      expect(db.prepare("select count(*) as count from posts where body = 'Recovered background result'").get())
        .toEqual({ count: 1 });
      expect(db.prepare("select count(*) as count from assistant_projections where pi_message_id = 'pi-recovered'").get())
        .toEqual({ count: 1 });
      expect(db.prepare("select count(*) as count from assistant_projections where pi_message_id in ('pi-live-a', 'pi-recovered', 'pi-lost-c')").get())
        .toEqual({ count: 3 });
  ...
  it('fails closed for a missing-ID history item without suppressing its valid sibling', async () => {
    ...
    const human = store.createIdentity('Author', 'human');
    store.createIdentity('Monika', 'robot');
    const { topic, post } = store.createTopic({ forumId: forum.id, title: 'Topic', body: 'Prompt', authorId: human.id });
    const session = store.ensureSession({ topicId: topic.id });
    store.upsertPiSessionLink({
      piSessionId: 'pi-parent', piSessionPath: '/tmp/parent.jsonl', topicId: topic.id, sessionId: session.id,
    });
    ...
    vi.spyOn((bridge as any).client, 'getConversationHistory').mockResolvedValue({
      conversation_id: 'conversation',
      items: [
        { id: 'wire-event-id', event: 'item_completed', data: {
          pi_message_id: '   ',
          item: {
            type: 'message', message_kind: 'assistant_outward', role: 'assistant',
            content: [{ type: 'text', text: 'Must not be projected' }],
          },
        } },
        { id: 'wire-valid-id', event: 'item_completed', data: {
          item: {
            id: 'pi-valid-sibling', message_kind: 'assistant_outward', type: 'message', role: 'assistant',
    ...
    const context = {
      topicId: topic.id, sessionId: session.id, activeThreadId: 'thread', lastUserPostId: post.id,
      turnParentPostId: post.id, planId: null, reasoningSummary: '', reasoningBackfillAttempted: false,
      reasoningBackfillRetries: 0, model: 'm', reasoningEffort: null, currentTurnId: null, turnStartedAt: Date.now(),
      lastUsage: null, totalInputTokens: 0, totalOutputTokens: 0, activeSubagents: new Map(),
      lastStreamEventAt: null, reasoningCheckpoints: [], assistantText: '', assistantCheckpoints: [],
    };
    ...
    expect(db.prepare("select count(*) as count from posts where body = 'Must not be projected'").get()).toEqual({ count: 0 });
