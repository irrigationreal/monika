# 200MiB Upload Repro (Playwright)

This repro uses the Playwright UI test runner to upload a 200MiB attachment via the New Thread flow.
...
pnpm e2e -- --grep "200MiB upload repro"
```

Optional environment variables:

- `E2E_FORUM_ID` – forum id to target directly (skips clicking the first forum row).
- `PW_UPLOAD_FILE` – pre-generated 200MiB file path (defaults to `/tmp/codex-forum-200mib.bin`).
...
3. Upload a 200MiB attachment and submit the thread.
4. Land on the topic page with the attachment link visible.

If you see failures:

- Verify server limits such as `CODEX_FORUM_MAX_ATTACHMENT_BYTES` and `CODEX_FORUM_MAX_REQUEST_BODY_BYTES` are at least 200MiB.
- Confirm any reverse proxy `client_max_body_size` or equivalent allows 200MiB uploads.
