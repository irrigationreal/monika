...
Forum changes are checked by `CI / Forum`, which follows the Vesper
branch-gate pattern: `forum-changes` decides whether the build is relevant,
`forum-build` runs the forum unit/E2E tests and container build, and
`forum-checks` is the stable required branch-protection check.
