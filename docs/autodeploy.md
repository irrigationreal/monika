...
For the single rollout from a forum image that predates admission, an exact `404` from acquire switches that attempt to bounded polling of the legacy authenticated quiescence endpoint, including a second check immediately before Compose. Other HTTP and transport failures remain closed. Once the new forum starts, later attempts use admission normally; the compatibility path cannot bypass a server that implements the endpoint.
