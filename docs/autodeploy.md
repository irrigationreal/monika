...
effects resolution; it must never be automatically replayed or dismissed. Agentd's
active-run and effects-unknown blockers preserve aggregate counts and attach at most
20 deterministic opaque run identities and execution/effects states, with
cap/truncation metadata; they never expose run paths, tasks, or output.
...
If the command exits `75`, do not force a restart. Inspect both forum deploy status and `curl -fsS http://127.0.0.1:7724/v1/admin/subagents`, wait for active work to finish, and retry. Quiescence blocker `items` are deterministic and capped at 20: forum actionable-dispatch items identify only topic/dispatch/session/status, non-idle robot items identify topic/session/activity, and agentd subagent items identify run key/ID and execution/effects state. The accompanying `item_limit`, `truncated`, and `omitted_count` fields distinguish a complete list from a capped one without exposing titles, content, actors, errors, origins, attachments, paths, tasks, or output. An `uncertain` run requires runtime/PID reconciliation or the audited quarantine procedure in `docs/redeployment.md`; an `effects_state: "unknown"` run requires remote-effect investigation and an audited effects attestation. Never remove lifecycle files merely to make either counter disappear.

Forum reconciles unresolved Stop Robot `stopping`/`uncertain` records at startup and in batches started about every 30 seconds. Each single-flight pass checks at most 20 oldest-attempted records concurrently through only agentd's canonical cancellation GET. Null/404 rotates without changing activity; results and failures apply only while the observed generation and unresolved state remain current. The pass never wakes work or loads a conversation and is joined during shutdown.
