...
`/v1/admin/quiescence` reports active turns, reconciled async-subagent execution, loaded conversations, interactive Pi ownership leases, memstore save queue state, and whether the container is immediately safe to stop. Active turns, active or uncertain subagent runners, interactive Pi sessions, and busy/unreachable memstore are hard blockers. Idle loaded conversations are not hard blockers, but they require a drain because closing them triggers Pi session shutdown and memory save. Claimed leases expire after a crashed terminal. First-action reservations for not-yet-materialized Pi sessions are also durable, renewable blockers once the launcher is used; an unused launcher creates no record. Deploy automation must not override either kind because recreating the container would terminate or fence that administrator session. Agentd `GET /healthz` deliberately does not perform an authoritative scan: its `interactive_pi_sessions` is an approximate cached count of pending and claimed records and may temporarily include expired ownership. Health uses bounded in-memory runtime state and last-successful lifecycle counts with explicit scan age so it remains responsive during canonical turns. Health is not evidence that the runtime is safe to stop; `/v1/admin/quiescence` freshly prunes ownership and is authoritative.

Subagent execution state comes from the durable run ledger below `/data/pi-subagents/async-subagent-runs`, not stale loaded-memory flags. Stop Robot uses the same authority by canonical Pi session, including unloaded parents: dispatch is fenced before agentd performs scoped top-level/nested fixed-point cancellation. A stop response distinguishes accepted/still `stopping`, proven-local `stopped`, and `uncertain`; cancelled result bytes remain retained, and remote `effects_state=unknown` remains a deployment blocker. Continue stays disabled until reconciliation proves the barrier resolved. Forum startup and a single-flight loop about every 30 seconds query only the canonical cancellation GET; they do not load/open conversations, create cancellation operations, consume results, or dispatch work. Matching-generation proof may advance the state; null/404 stays ambiguous and transport failure retains `uncertain`. Forum shutdown clears and joins the loop before its database closes. Scheduled package runs remain disabled and are not silently included in the cancellation claim. `GET /v1/admin/subagents` exposes a capped operational summary. Exact observed terminal proof, same-container PID/start-identity reconciliation, and container runtime epochs distinguish active, terminal, interrupted, and uncertain work. Pending result projection is durable delivery work, not a live execution blocker. Effects-unknown also does not claim a live process, but it blocks safe deployment and prohibits automatic replay or unaudited delivery dismissal until reconciled or operator-attested. Lifecycle traversal errors fail closed as `subagent_lifecycle_unavailable`. Active-run and effects-unknown quiescence blockers preserve `code`/`count` and include at most 20 deterministic opaque items containing only `run_key`, `run_id`, execution state, and effects state, plus cap/truncation metadata; tasks, output, and paths are excluded.
...
5. If the pulled image IDs already match the running containers, proves Forum `/readyz` before exiting so a retry cannot hide an unresolved post-replacement readiness failure.
...
11. Renews forum admission immediately before the first replacement. Hosts with `MONIKA_PUBLIC_INGRESS=1` reconcile the digest-pinned `cloudflared` connector independently before image comparison.
12. If Monika changed, replaces only `monika` with `--no-deps`. The replacement restores the unexpired durable drain; the script cancels it on the new agentd and waits for healthy/undrained state.
13. If Forum also changed, renews admission against the still-running old forum after agentd recovery, then replaces only `forum` with `--no-deps`. This staged joint update preserves Forum's healthy-agentd Compose dependency without waiting on a drained Monika before the script can cancel that drain.
14. After every applied Monika or forum image change—including forum-only updates—waits a bounded interval for the forum's exact unprefixed `/readyz`. A readiness failure aborts before backup/image pruning.
15. Cancels/reopens forum admission after readiness (or after backup-only completion); the exit trap attempts both forum cancellation and agentd drain cancellation on every abort.
16. Prunes old redeploy backups by tiered retention bucket.
17. Prunes old dangling Docker image layers conservatively.
...
An exact `404` from admission acquire identifies the one-release bootstrap from a pre-admission forum image. Only then, the host boundedly polls the legacy authenticated quiescence snapshot initially and immediately before each replacement boundary—twice for a one-service update, or three times for a staged joint update. Any other failure remains closed; after replacement, admission is mandatory.
